	...
	$('#selectFile').dialog({
	    ...
	    buttons:[{
			...
			handler:function(){
				...
				var data=execJava("model.existsFile",{fileListJson:JSON.stringify(list)});
				...
				if(data.suc){
					...
					if($r.length>0){//包含已存在的文件
						...
						$.messager.confirm(infoTitle,$info,function(r){
							if (r){
								data=execJava("model.createObjFile",{fileListJson:JSON.stringify(list),wordListJson:JSON.stringify(modelDeal[selectModelIndex].specialWords)});
								if(data.msg){
									infoMsg(data.msg);
								}
							}
...
function addModelFile(){
	...
	var r=chooseFile('mfile','选择文件',modelDeal[selectModelIndex].folderPath);
...
function saveData(msg){
	var data=execJava('writeAppData',{appId:appId,data:JSON.stringify(modelDeal)});
	if(!data.suc){
		infoMsg(data.msg);
	}else{
		if(msg){
...
function readData(func){
	var data=execJava('readAppData',{appId:appId});
	if(data.data){
		modelDeal=JSON.parse(data.data);
	}
